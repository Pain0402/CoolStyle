...
- [x] Task 5: Frontend - Giao diện Đánh giá Sản phẩm -> Verify: Trên trang chi tiết sản phẩm, hiển thị sao đánh giá, danh sách review và form gửi review.
- [x] Task 6: Frontend - Tích hợp checkout thanh toán -> Verify: Thêm lựa chọn phương thức thanh toán tại giỏ hàng, chuyển hướng sang page VNPAY và xử lý màn hình kết quả thanh toán.
- [x] Task 7: Frontend - Dashboard Admin Báo cáo -> Verify: Trang Admin Dashboard hiển thị biểu đồ thống kê doanh thu (dùng Chart.js/Recharts), số lượng đơn hàng và đánh giá.

## Done When
- [x] Người dùng hoàn tất quá trình thanh toán qua VNPAY, đơn hàng được cập nhật "Đã thanh toán".
- [x] Admin truy cập Dashboard xem được tổng quan doanh thu, số đơn ngay trên hình ảnh trực quan.
- [x] Người dùng truy cập sản phẩm xem được các nhận xét kèm số sao trung bình trên Giao diện sản phẩm.
